                                ...
                                <tr v-for="(item,index) of list" class="list-con-item">
                                    ...
                                    <td>
                                        <select class="width80" name="" v-model="item.communicatemodel" v-if="showSelect">
                                            <option :value="item1.model" v-for="item1 of module['type' + (index+2)]">{{item1.model}}</option>
                                        </select>
...
export default {
    ...
    watch: {
        ...
        'showSelectList': {
            handler: function (val, oldVal) {
                if (this.showSelectList.length == this.list.length) {
    ...
    created() {
        ...
        if (this.secondAdd.type == 2) {
            ...
            this.getModuleById(_list.moduleid).then(()=>{
                this.findModuleInfoById(_list.id)
            });
        } else {
            this.findAllModuleModel().then((res)=>{
                for (let i = 0; i < res.length; i++) {
                    this.getModuleById(res[i].moduleid);
                }
